...
import { DBManagerFactory } from "../../../db-layer/DataAccessLayerFactory";
 import { CustomDiscount, CustomDiscountItems } from "../../models/ecommerce/CustomerOrderModel";
import { OrderProductsModel } from "../../models/ecommerce/OrderProductsModel";
import { CartResponseModel } from "../../models/ecommerce/response/CartResponseModel";
import { ObjectStatus } from "../../models/ObjectStatus";
import { ResultModel } from "../../models/ResultModel";


export class FuelDiscountService implements IFuelDiscountService {
    private readonly discountDBManager: IFuelDiscountDBManager;

    constructor() {
        this.discountDBManager = DBManagerFactory.getFuelDiscountDBManager();
    ...
    public async getDiscountCouponById(fuelStationId: string, couponId: string): Promise<FuelDiscountCouponModel> {
        return await this.discountDBManager.getDiscountCouponById(fuelStationId, couponId);
    }
    public async addOrUpdateDiscountCoupon(promotion: FuelDiscountCouponModel): Promise<FuelDiscountCouponModel> {

        let req = new GetFuelDiscountCouponListRequestModel();
        req.fuelStationId = promotion.getFuelStationId();
        ...
        let savedPromotion: FuelDiscountCouponModel = await this.discountDBManager.addOrUpdateDiscountCoupon(promotion);
...
/*
    ...
    public async applyCouponPromotionV1(order: CustomerOrderModel, couponCode: string, throwUpdateOrder: boolean, couponId?: string): Promise<any> {
        ...
        let promotion: FuelDiscountCouponModel = await this.discountDBManager.getDiscountCouponForBusinessByCouponCode(order.getBusinessId(), couponCode);
